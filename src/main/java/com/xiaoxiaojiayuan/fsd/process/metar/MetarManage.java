...
package com.xiaoxiaojiayuan.fsd.process.metar;
...
import com.xiaoxiaojiayuan.fsd.constants.MetarSource;
import com.xiaoxiaojiayuan.fsd.constants.ServerConstants;
...
public class MetarManage extends Process {
    ...
    private BufferedWriter ioIn;

    private BufferedReader ioOut;

    private Socket sock;

    private BufferedReader dataReadSock;

    private BufferedWriter dataSock;
    ...
    private int varTotal;

    private int varStations;
    ...
    private void parseWeatherConfig() {
        ConfigGroup weatherGroup = Fsd.configManager.getGroup("weather");
        ...
        if (sourceEntry != null) {
            ...
            if ("network".equals(source)) {
                ...
                LOGGER.error(String.format("Unknown METAR source %s in config file", source));
            }
        }
    ...
    private void parseSystemConfig() {
        ConfigGroup systemGroup = Fsd.configManager.getGroup("system");
    ...
    private void parseMetar() {
        ...
        try (InputStreamReader read = new InputStreamReader(new FileInputStream(file));
            ...
            LOGGER.error("Config file not found: " + FsdPath.METARFILE);
        } catch (IOException e) {
            LOGGER.error("Something went wrong when parse metar file: ", e);
        }
    }
    ...
    private void doDownload() {
        ...
        if (now - prevDownload > MAX_METAR_DOWNLOAD_TIME) {
            LOGGER.warn("METAR download interrupted due to timeout");
            stopDownload();
            startDownload();
            return;
        }
        try {
            if (dataReadSock.ready()) {
    ...
    private int doParse(Mmq q) {
        ...
        if (index < 0) {
            Fsd.serverInterface.sendNoWx(q.getDestination(), q.getFd(), q.getMetarId());
            delQ(q);
            return 1;
        }
        ...
        try {
            ioOut = new BufferedReader(new InputStreamReader(new FileInputStream(new File(FsdPath.METARFILE))));
        } catch (IOException e) {
        ...
        if (q.isParsed() == 1) {
            ...
            Fsd.serverInterface.sendWeather(q.getDestination(), q.getFd(), profile);
        } else {
            Fsd.serverInterface.sendMetar(q.getDestination(), q.getFd(), location.getName(), line);
        }
    ...
    public Server requestMetar(String client, String metar, int parsed, int fd) {
        if (source == MetarSource.SOURCE_NETWORK) {
            int hops = -1;
            Server best = null;
            ...
            Fsd.serverInterface.sendReqMetar(client, metar, fd, parsed, best);
            return best;
        } else {
    ...
    private void addQ(String dest, String metar, int parsed, int fd) {
        WProfile prof = Weather.getWProfile(metar);
        if (prof != null && prof.getActive() == 1) {
            if (parsed == 1) {
                Fsd.serverInterface.sendWeather(dest, fd, prof);
            } else {
                Fsd.serverInterface.sendMetar(dest, fd, metar, prof.getRawCode());
            }
            return;
        }
        Mmq temp = new Mmq();
