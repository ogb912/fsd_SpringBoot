...
import com.xiaoxiaojiayuan.fsd.constants.ClientConstants;
import com.xiaoxiaojiayuan.fsd.constants.GlobalConstants;
import com.xiaoxiaojiayuan.fsd.constants.NetworkConstants;
import com.xiaoxiaojiayuan.fsd.constants.ProtocolConstants;
import com.xiaoxiaojiayuan.fsd.model.Client;
import com.xiaoxiaojiayuan.fsd.model.Flightplan;
import com.xiaoxiaojiayuan.fsd.model.Server;
import com.xiaoxiaojiayuan.fsd.user.AbstractUser;
import com.xiaoxiaojiayuan.fsd.user.ClientUser;
import com.xiaoxiaojiayuan.fsd.support.Support;
import com.xiaoxiaojiayuan.fsd.weather.CloudLayer;
import com.xiaoxiaojiayuan.fsd.weather.TempLayer;
import com.xiaoxiaojiayuan.fsd.weather.WProfile;
import com.xiaoxiaojiayuan.fsd.weather.WindLayer;
...
public class ClientInterface extends TcpInterface {
    ...
    public int getBroad(String s) {
        ...
        if ("*P".equals(s)) {
            broad = ClientConstants.CLIENT_PILOT;
        } else if ("*A".equals(s)) {
            broad = ClientConstants.CLIENT_ATC;
        }
        return broad;
    }
...
//dolog(L_INFO,"SendPilotPos is sending: %s",data);
    ...
    public int calcRange(Client from, Client to, int type, int range) {
        ...
        switch (type) {
            ...
            case ProtocolConstants.CL_ATCPOS:
                if (to.getType() == ClientConstants.CLIENT_ATC) {
                    return to.getVisualRange();
                }
                x = to.getRange();
                y = from.getRange();
                if (from.getType() == ClientConstants.CLIENT_PILOT) {
                    return x + y;
                }
                return Math.max(x, y);
    ...
    */
    public void sendPacket(Client dest, Client source, AbstractUser exclude,
        ...
        if (dest != null) {
            if (dest.getLocation() != Server.myServer) {
                return;
            }
        }
        for (AbstractUser temp : users) {
            if (temp.getKillFlag() == 0) {
                ...
                if (dest != null && cl != dest) {
                    continue;
                }
                if ((cl.getType() & broad) == 0) {
                    continue;
                }
                if (source != null && (range != -1 || cmd == ProtocolConstants.CL_PILOTPOS || cmd == ProtocolConstants.CL_ATCPOS)) {
                    ...
                    if (distance == -1 || distance > checkRange) {
                        continue;
                    }
                }
