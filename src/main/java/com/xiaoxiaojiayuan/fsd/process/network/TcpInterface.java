...
import com.xiaoxiaojiayuan.fsd.constants.GlobalConstants;
import com.xiaoxiaojiayuan.fsd.constants.ManageVarType;
import com.xiaoxiaojiayuan.fsd.constants.NetworkConstants;
...
import com.xiaoxiaojiayuan.fsd.user.AbstractUser;
...
public class TcpInterface extends Process {
    ...
    public TcpInterface(int port, String code, String desc) {
        ...
        try {
            ...
            LOGGER.error("Could not open server socket channel.", e);
            System.exit(1);
        }
        ...
        try {
            ...
            LOGGER.error(String.format("setsockopt error on port %d", port), e);
            System.exit(1);
        } catch (UnsupportedOperationException e) {
            LOGGER.warn("SO_REUSEADDR is not supported on current platform");
        }

        try {
            sock.setOption(StandardSocketOptions.TCP_NODELAY, on);
        } catch (IOException e) {
            LOGGER.error(String.format("Could not set TCP_NODELAY on port %d", port), e);
            System.exit(1);
        } catch (UnsupportedOperationException e) {
            LOGGER.warn("TCP_NODELAY is not supported on current platform");
        }
        ...
        try {
            ...
            LOGGER.error(String.format("Bind error on port %d", port), e);
            System.exit(1);
        }
        ...
        feedStrategy = 0;
        prevChecks = 0;
        LOGGER.info(String.format("Booting port %d (%s)", port, description));
    }
    ...
    public void close() {
        try {
            ...
            LOGGER.warn("Close tcpInterface error.");
        }
    }
    ...
    public void addGuard(AbstractUser who) {
    ...
    public void allow(String name) {
        try {
            ...
            LOGGER.error(String.format("Server %s unknown in allowfrom", name));
        }
    }

    public void newUser(SelectionKey key) {
        String peer;
        ...
        try {
            ...
            if (ok == 0 && !allows.isEmpty()) {
                ...
                LOGGER.info(String.format("Connection rejected from %s", peer));
                clientChannel.close();
                return;
            }
        } catch (IOException e) {
            LOGGER.error("Accept connection failed.", e);
            return;
        }

        LOGGER.info(String.format("Connection accepted from %s on %s", peer, description));

        try {
            clientChannel.setOption(StandardSocketOptions.TCP_NODELAY, true);
        } catch (IOException e) {
            LOGGER.error("Could not set off TCP_NODELAY");
        } catch (UnsupportedOperationException e) {
            LOGGER.warn("TCP_NODELAY is not supported on current platform");
        }
    ...
    public void insertUser(AbstractUser user) {
        ...
        int peak = Manage.manager.getVar(varPeak).getValue().getNumber();
        int now = Manage.manager.getVar(varCurrent).getValue().getNumber();
        if (now > peak) {
    ...
    public boolean run() {
        ...
        try {
            ...
            LOGGER.error("run select failed", e);
        }
    ...
    public int addUser(String name, int port, AbstractUser terminal) {
        SocketChannel sock;
        ...
        try {
            ...
            LOGGER.error("Could not open socket channel", e);
            return 0;
        }
