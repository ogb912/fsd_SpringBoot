...
import com.xiaoxiaojiayuan.fsd.process.network.ServerInterface;
...
public class ServerUser extends AbstractUser {
    ...
    private int execPlan(String[] array, int count) {
        ...
        if (who.getPlan() != null && who.getPlan().getRevision() >= revision) {
            return 0;
        }
    ...
    private void execAddWp(String[] array, int count) {
        ...
        if (count < 57 || MetarManage.metarManager.getSource() == MetarSource.SOURCE_NETWORK) {
            return;
        }
        ConfigGroup group = Fsd.configManager.getGroup("hosts");
        ConfigEntry entry = null;
    ...
    private void execDelWp(String[] array, int count) {
        if (count < 5 || MetarManage.metarManager.getSource() == MetarSource.SOURCE_NETWORK) {
            return;
        }
        ConfigGroup group = Fsd.configManager.getGroup("hosts");
        ConfigEntry entry = null;
