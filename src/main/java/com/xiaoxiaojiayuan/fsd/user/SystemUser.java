package com.xiaoxiaojiayuan.fsd.user;

import com.xiaoxiaojiayuan.fsd.Fsd;
import com.xiaoxiaojiayuan.fsd.constants.*;
import com.xiaoxiaojiayuan.fsd.constants.*;
import com.xiaoxiaojiayuan.fsd.manager.Manage;
import com.xiaoxiaojiayuan.fsd.model.Certificate;
import com.xiaoxiaojiayuan.fsd.model.Client;
import com.xiaoxiaojiayuan.fsd.model.Server;
import com.xiaoxiaojiayuan.fsd.process.config.ConfigEntry;
import com.xiaoxiaojiayuan.fsd.process.config.ConfigGroup;
import com.xiaoxiaojiayuan.fsd.process.metar.MetarManage;
import com.xiaoxiaojiayuan.fsd.process.network.Guard;
import com.xiaoxiaojiayuan.fsd.process.network.SystemInterface;
import com.xiaoxiaojiayuan.fsd.support.Support;
import com.xiaoxiaojiayuan.fsd.weather.WProfile;
import com.xiaoxiaojiayuan.fsd.weather.Weather;
...
public class SystemUser extends AbstractUser {
    ...
    private SystemInterface parent;
    ...
    public SystemUser(SocketChannel fd, SystemInterface p, String pn, int portNum, int g) {
        ...
        ConfigGroup group = Fsd.configManager.getGroup("system");
    ...
    private void runCmd(int num, String[] array, int count) {
        switch (num) {
            case SystemConstants.SYS_SERVERS:
                execServers(array, count, 1);
                break;
            case SystemConstants.SYS_SERVERS2:
                execServers(array, count, 2);
                break;
            case SystemConstants.SYS_INFORMATION:
                information();
                break;
            case SystemConstants.SYS_PING:
                execPing(array, count);
                break;
            case SystemConstants.SYS_CONNECT:
                execConnect(array, count);
                break;
            case SystemConstants.SYS_ROUTE:
                execRoute(array, count);
                break;
            case SystemConstants.SYS_DISCONNECT:
                execDisconnect(array, count);
                break;
            case SystemConstants.SYS_HELP:
                execHelp(array, count);
                break;
            case SystemConstants.SYS_QUIT:
                kill(NetworkConstants.KILL_COMMAND);
                break;
            case SystemConstants.SYS_STAT:
                list((count == 0 ? "" : array[0]));
                break;
            case SystemConstants.SYS_SAY:
                execSay(array, count);
                break;
            case SystemConstants.SYS_CLIENTS:
                execClients(array, count);
                break;
            case SystemConstants.SYS_CERT:
                execCert(array, count);
                break;
            case SystemConstants.SYS_DISTANCE:
                execDistance(array, count);
                break;
            case SystemConstants.SYS_TIME:
                execTime(array, count);
                break;
            case SystemConstants.SYS_WEATHER:
                execWeather(array, count);
                break;
            case SystemConstants.SYS_RANGE:
                execRange(array, count);
                break;
            case SystemConstants.SYS_LOG:
                execLog(array, count);
                break;
            case SystemConstants.SYS_PWD:
                execPwd(array, count);
                break;
            case SystemConstants.SYS_WALL:
                execWall(array, count);
                break;
            case SystemConstants.SYS_DELGUARD:
                execDelGuard(array, count);
                break;
            case SystemConstants.SYS_METAR:
                execMetar(array, count);
                break;
            case SystemConstants.SYS_WP:
                execWp(array, count);
                break;
            case SystemConstants.SYS_KILL:
                execKill(array, count);
                break;
            case SystemConstants.SYS_DUMP:
                execDump(array, count);
                break;
            case SystemConstants.SYS_POS:
                execPos(array, count);
                break;
            case SystemConstants.SYS_REFMETAR:
                execRefMetar(array, count);
                break;
        }
    }
    ...
    private void execConnect(String[] array, int count) {
        if (count < 1) {
            ...
            List<Guard> guards = Fsd.serverInterface.getGuards();
            if (guards.size() == 0) {
        ...
        int port = 3011;
        if (count > 1) {
            port = NumberUtils.toInt(array[1]);
        }
        if (Fsd.serverInterface.addUser(array[0], port, this) == 1) {
            Fsd.serverInterface.sendServerNotify("*", Server.myServer, null);
        }
    }
    ...
    private void execPing(String[] array, int count) {
        ...
        Fsd.serverInterface.sendPing(array[0], data);
    }
    ...
    private void execServers(String[] array, int count, int type) {
        ...
        if (count == 1) {
            ...
            if (temp == null && match != null) {
                ...
                if ((match.getFlags() & ServerConstants.SERVER_SILENT) != 0) {
                    flags += "SILENT";
                }
                if ((match.getFlags() & ServerConstants.SERVER_METAR) != 0) {
    ...
    private void execSay(String[] array, int count) {
        if (count < 2) {
        ...
        if (Fsd.serverInterface.sendMulticast(null, array[0], buf.toString(), ProtocolConstants.CL_MESSAGE, 1, null) == 0) {
    ...
    private void execClients(String[] array, int count) {
        ...
        if (count > 0) {
            ...
            if (temp == null) {
                ...
                uprintf("Callsign    : %s\r\n", match.getCallsign());
                if (StringUtils.isNotEmpty(match.getRealName())) {
                    uprintf("Real name   : %s\r\n", match.getRealName());
                }
                uprintf("Type        : %s\r\n", match.getType() == ClientConstants.CLIENT_ATC ?
                        "atc" : "pilot");
                uprintf("Online      : %s\r\n", Support.sprintTime(Support.mtime() - match.getStartTime()));
                uprintf("Idle        : %s\r\n", Support.sprintTime(Support.mtime() - match.getAlive()));
                if (match.getFrequency() != 0 && match.getFrequency() < 100000) {
                    uprintf("Frequency   : 1%02d.%03d\r\n", match.getFrequency() / 1000, match.getFrequency() % 1000);
                }
                if (match.getPositionOk() == 1) {
                    uprintf("Location    : %s\r\n", Support.printLoc(match.getLat(), match.getLon()));
                    uprintf("Altitude    : %d\r\n", match.getAltitude());
                    if (match.getType() == ClientConstants.CLIENT_PILOT) {
                        uprintf("Groundspeed : %d\r\n", match.getGroundSpeed());
                        uprintf("Transponder : %d\r\n", match.getTransponder());
                    } else {
                        uprintf("Facility    : %d\r\n", match.getFacilityType());
                    }
                }
                return;
            }
    ...
    private void execCert(String[] array, int count) {
        ...
        switch (mode) {
            case ProtocolConstants.CERT_ADD:
                ...
                Fsd.serverInterface.sendCert("*", mode, c, null);
                break;
...
//        }
    ...
    void execPwd(String[] array, int count) {
        ...
        ConfigGroup sgroup = Fsd.configManager.getGroup("system");
    ...
    void execWall(String[] array, int count) {
        ...
        for (AbstractUser temp : Fsd.clientInterface.getUsers()) {
            ClientUser ctemp = (ClientUser) temp;
            Client c = ctemp.getThisClient();
            if (c == null) {
                continue;
            }
            Fsd.clientInterface.sendGeneric(c.getCallsign(), c, null, null, "server", buf, ProtocolConstants.CL_MESSAGE);
        }
    }

    void execDelGuard(String[] array, int count) {
        Fsd.serverInterface.delGuard();
    ...
    void execWp(String[] array, int count) {
        String mode = array[0];
        ...
        if ("delete".equalsIgnoreCase(mode)) {
            if (wp.getActive() == 1) {
                Fsd.serverInterface.sendDelWp(wp);
            }
            wp.close();
            uprintf("Weather profile deleted.\r\n");
        } else if ("activate".equalsIgnoreCase(mode)) {
            wp.activate();
            wp.genRawCode();
            Fsd.serverInterface.sendAddWp(null, wp);
    ...
    void execKill(String[] array, int count) {
        if (count < 2) {
        ...
        Fsd.serverInterface.sendKill(c, buf);
        uprintf("Killed\r\n");
    }
    ...
    void execDump(String[] array, int count) {
        ...
        for (AbstractUser temp : Fsd.serverInterface.getUsers()) {
            if (temp.fd == num) {
    ...
    public void printWeather(WProfile wp) {
        uprintf("\r\nWeather profile %s:\r\n\r\n", wp.getName());
        int x;
        StringBuilder line;
        String buf;
    ...
    void doSet(WProfile wp, String p, int val) {
        int x, ok;
        String[] cat1 = {
        ...
        switch (x) {
            case 0:
                switch (y) {
                    case 0:
                        wp.getWinds().get(index).setFloor(val);
                        break;
                    case 1:
                        wp.getWinds().get(index).setCeiling(val);
                        break;
                    case 2:
                        wp.getWinds().get(index).setDirection(val);
                        break;
                    case 3:
                        wp.getWinds().get(index).setSpeed(val);
                        break;
                    case 4:
                        wp.getWinds().get(index).setGusting(val > 0 ? 1 : 0);
                        break;
                }
                break;
            case 1:
                switch (y) {
                    case 0:
                        wp.getClouds().get(index).setFloor(val);
                        break;
                    case 1:
                        wp.getClouds().get(index).setCeiling(val);
                        break;
                    case 2:
                        wp.getClouds().get(index).setCoverage(val);
                        break;
                    case 3:
                        wp.getClouds().get(index).setTurbulence(val);
                        break;
                    case 4:
                        wp.getClouds().get(index).setIcing(val > 0 ? 1 : 0);
                        break;
                }
                break;
            case 2:
                switch (y) {
                    case 0:
                        wp.getTemps().get(index).setCeiling(val);
                        break;
                    case 1:
                        wp.getTemps().get(index).setTemp(val);
                        break;
                }
                break;
        }
