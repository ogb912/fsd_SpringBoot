...
import com.xiaoxiaojiayuan.fsd.process.network.ClientInterface;
import com.xiaoxiaojiayuan.fsd.process.network.TcpInterface;
...
public class ClientUser extends AbstractUser {
    ...
    public int callsignOk(String name) {
        if (name.length() < 2 || name.length() > GlobalConstants.CALLSIGN_BYTES) {
            return ProtocolConstants.ERR_CSINVALID;
        }
        if (StringUtils.indexOfAny(name, "!@#$%*:& \t") != -1) {
            return ProtocolConstants.ERR_CSINVALID;
        }
        for (Client client : Client.clients) {
            if (client.getCallsign().equals(name)) {
                return ProtocolConstants.ERR_CSINUSE;
            }
        }
