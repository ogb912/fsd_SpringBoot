...
public class AbstractUser {
    ...
    public void close() {
        LOGGER.info(String.format("client from %s removed from %s:%s", peer, baseParent.getDescription(), NetworkConstants.KILL_REASONS[killFlag]));
        ...
        try {
            ...
            LOGGER.warn(String.format("Close client %s failed.", peer), e);
        }
    }
    ...
    public void setMasks(Set<SelectableChannel> rmask, Set<SelectableChannel> wmask) {
    ...
    private int nextLine(StringBuilder dest) {
        if (!inBuf.hasArray()) {
            return -1;
        }
    ...
    public void uprintf(String format, Object... args) {
        if (killFlag != 0) {
            return;
        }
    ...
    public void uslprintf(String format, int limit, Object... args) {
        if (killFlag != 0) {
            return;
        }
